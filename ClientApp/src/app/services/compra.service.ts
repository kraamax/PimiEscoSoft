...
export class CompraService {
  ...
  addCompra (compra: Compra): Observable<Compra> {
    return this.http.post<Compra>(this.baseUrl+'api/facturaDetalle', compra, httpOptions).pipe(
  ...
  getAll(): Observable<Compra[]> {
    return this.http.get<Compra[]>(this.baseUrl + 'api/facturaDetalle').pipe(
...
{
const url = `${this.baseUrl + 'api/facturaDetalle'}/${id}`;
...
update (compra: Compra): Observable<any> {
  const url = `${this.baseUrl + 'api/facturaDetalle'}/${compra.id}`;
  ...
  delete (compra: Compra | number): Observable<Compra> {
    ...
    tap(_ =>console.log(`deleted Compra id=${id}`)),
